...
const SKIP_MACROS = new Set(["addlinespace", "rowcolor", "noalign"]);
...
function splitRows(content: Ast.Node[]): { cells: Ast.Node[][]; ruleAbove: boolean }[] {
  const rows: { cells: Ast.Node[][]; ruleAbove: boolean }[] = [];
  let cells: Ast.Node[][] = [[]];
  let ruleNext = false;
  ...
  const finishRow = () => {
    ...
    if (!isBlank) rows.push({ cells, ruleAbove: ruleNext });
    cells = [[]];
    ruleNext = false;
  };
...
function buildCell(nodes: Ast.Node[], ctx: Ctx, colAlign: Align[], colIndex: number, ruleAbove: boolean): TableCell {
  ...
  return { content, align, colspan, rowspan: 1, ruleAbove: ruleAbove || undefined };
}
...
export function parseTabular(env: Ast.Environment, ctx: Ctx): { rows: TableRow[]; columnAlign: Align[] } {
  ...
  const rows: TableRow[] = rawRows.map(({ cells, ruleAbove }) => {
    const out: TableCell[] = [];
    let col = 0;
    for (const cnodes of cells) {
      const cell = buildCell(cnodes, ctx, columnAlign, col, ruleAbove);
      out.push(cell);
      col += cell.colspan;
    }
    return out;
